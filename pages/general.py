...
if __name__ == '__main__':
    ...
    st.markdown("<h2 style='text-align: center;'> Médias </h2>", unsafe_allow_html=True)

    with st.container():
        col_1, col_2, col_3 = st.columns(3)

        with col_1:
            st.markdown("<h3 style='text-align: center;'>Pontos</h3>", unsafe_allow_html=True)
            st.markdown(create_card(calculate_mean_est(df_filtered, 'pts_per_season'), AZUL_NBA), unsafe_allow_html=True)

        with col_2:
            st.markdown("<h3 style='text-align: center;'>Assistências</h3>", unsafe_allow_html=True)
            st.markdown(create_card(calculate_mean_est(df_filtered, 'ast_per_season'), VERMELHO_NBA), unsafe_allow_html=True)

        with col_3:
            st.markdown("<h3 style='text-align: center;'>Rebotes</h3>", unsafe_allow_html=True)
